    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
      {data.map((user) => (
        ...
        >
          ...
          {openModal && (
            <div className="mt-10 fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
              <div className="bg-white p-6 rounded-lg shadow-lg w-11/12 md:w-3/4 lg:w-1/2 max-h-[80vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">
                      ₹{user.investmentrange.min} - ₹{user.investmentrange.max}
                    </p>
                  </div>
                ...
                {user.numberhide ? (
                  ...
                  <p className="mt-4 text-gray-700 font-semibold">
                    Phone: {user.phone.$numberLong}
                  </p>
                )}

                {/* Close Button */}
                <button
                  className="mt-4 bg-yellow-400 text-black px-4 py-2 rounded hover:bg-yellow-200 w-full"
                  onClick={() => setOpenModal(false)}
                >
                  Close
                </button>
                <a href="/pricing">
                  <button className="mt-4 bg-yellow-400 text-black px-4 py-2 rounded hover:bg-yellow-200 w-full">
                    Buy
                  </button>
                </a>
              </div>
